...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,C2f_CBAM,C2f_ECA,C2f_SE,C2f_CA,C2f_SimAM):
            ...
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        elif m is LightASFF2:
            c1, c2 = [ch[f[0]], ch[f[1]]], args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            level = args[1]
            act = args[2] if len(args) > 2 else 'silu'
            args = [c1, c2, level, act]
        elif m is LightASFF3:
            c1, c2 = [ch[f[0]], ch[f[1]], ch[f[2]]], args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            level = args[1]
            act = args[2] if len(args) > 2 else 'silu'
            args = [c1, c2, level, act] 
        elif m is CSA_DW_LightASFF2:
            c1, c2 = [ch[f[0]], ch[f[1]]], args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            level = args[1]
            args = [c1, c2, level]
        elif m is CSA_DW_LightASFF3:
            c1, c2 = [ch[f[0]], ch[f[1]], ch[f[2]]], args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            level = args[1]
            args = [c1, c2, level] 
